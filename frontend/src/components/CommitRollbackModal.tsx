...
import { AlertTriangle, CheckCircle, GitCommit, Loader2, RotateCcw } from "lucide-react";
...
export default function CommitRollbackModal({
  ...
  const isLatest = commit?.hash === "latest";
  ...
  return (
    <ProjectModal
      aberto={aberto}
      titulo={isLatest ? "Voltar ao Último Commit" : "Resgatar Commit"}
          ...
          <div className="space-y-4">
            {isLatest ? (
              <div className="rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-4">
                ...
                <p className="mt-1 text-sm text-emerald-300/80">
                  O projeto será alterado para o último commit do branch
                  principal (main/master). Todas as alterações locais serão
                  descartadas.
                </p>
              </div>
            ) : (
            ...
            <div className="flex items-start gap-3 rounded-lg border border-amber-500/30 bg-amber-500/10 p-4">
              ...
              <div className="text-sm text-amber-200">
                ...
                <p className="mt-1 text-amber-300/80">
                  {isLatest ? (
                    <>
                      O projeto voltará ao estado do último commit do branch
                      principal.
                    </>
                  ) : (
                    <>
                      O projeto será alterado para o estado deste commit. Esta é
                      uma operação <strong>isolada</strong> (detached HEAD) — o
                      branch principal não será alterado. Para voltar ao estado
                      atual, use o botão{" "}
                      <strong>"Voltar ao último commit"</strong> no modal de
                      commits.
                    </>
                  )}
                </p>
            ...
            <label className="flex cursor-pointer items-center gap-3">
              ...
              <span className="text-sm text-slate-300">
                {isLatest
                  ? "Eu entendo que o projeto voltará ao branch principal"
        ...
        <div className="space-y-4">
          <div className="flex flex-col items-center gap-3 py-4">
            ...
            <p className="text-center text-sm text-slate-200">
              {isLatest
                ? "Projeto voltou ao branch principal!"
